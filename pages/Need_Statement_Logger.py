...
def extractObservationFeatures(observation):
    ...
    output = "\n".join([f"**{field.replace('_', ' ').capitalize()}**: {parsed_observation[field]}" for field in input_fields if field not in missing_fields])
    output += "\n\n **Missing fields**: " + ", ".join([f"<span style='color:red;'>{field.replace('_', ' ').capitalize()}</span>" for field in missing_fields])
...
def embedObservation(observer, observation, observation_summary, observation_date, observation_id):
    ...
    parsed_observation = parseObservation(observation)
    observation_keys = ['observation_summary', 'observer', 'observation', 'observation_date', 'observation_id'] + list(ObservationRecord.__fields__.keys())
    observation_values = [observation_summary, observer, observation, observation_date, observation_id] + [parsed_observation[key] for key in ObservationRecord.__fields__.keys()]
...
def clear_observation():
    for key in ['observation', 'observation_summary', 'result']:
...
if st.session_state['observation_summary']:
    st.text_area("Generated Summary (editable):", value=st.session_state['observation_summary'], height=50)
...
if st.button("Add Observation to Team Record", disabled=st.session_state['observation_summary'] == ""):
    if not st.session_state['observation']:
        st.markdown("<span style='color:red;'>Error! Please enter observation first</span>", unsafe_allow_html=True)
    elif not st.session_state['observation_summary']:
        st.markdown("<span style='color:red;'>Error! Please evaluate observation first</span>", unsafe_allow_html=True)
    else:
        if embedObservation(observer, st.session_state['observation'], st.session_state['observation_summary'], st.session_state['observation_date'], st.session_state['observation_id']):
