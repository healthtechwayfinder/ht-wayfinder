...
# Custom CSS to adjust the width of the dataframe
st.markdown(
    """
    <style>
    .dataframe-container {
...
for index, case in recent_cases.iterrows():
    ...
    if not relevant_observations.empty:
        ...
        # Add a 'Reviewed' column initialized to False (editable checkbox)
        relevant_observations['Reviewed'] = relevant_observations['Observation ID'].apply(lambda obs_id: st.session_state.get(f"reviewed_{obs_id}", False))

        # Use the custom dataframe-container class to adjust the width
        ...
        for obs in relevant_observations.itertuples():
            st.session_state[f"reviewed_{obs._1}"] = obs.Reviewed
