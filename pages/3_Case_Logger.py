...
def extractCaseFeatures(case_description):
    ...
    output = ""
    missing_fields = [field for field in caseRecord.__fields__.keys() if parsed_case[field] is None]
...
def addToGlossary(insider_language_terms, case_id):
    try:
        ...
        for term in terms_list:
            for i, row in enumerate(glossary_data, start=2):
                if row['Term'].strip().lower() == term.strip().lower():
                    if case_id not in row['Related cases'].split(', '):
                        glossary_sheet.update_cell(i, related_cases_col_idx, f"{row['Related cases']}, {case_id}")
                    break
            else:
                glossary_sheet.append_row([term, generateDefinition(term), case_id])
...
def embedCase(attendees, case_description, case_title, case_date, case_ID):
    ...
    case_dict = dict(zip(
        ['Title', 'People Present', 'Case Description', 'Date', 'Case ID', 'Attendees'] + [i.replace("_", " ").title() for i in caseRecord.__fields__.keys()],
        [case_title, attendees, case_description, case_date, case_ID, ', '.join(attendees)] + [parsed_case[key] for key in caseRecord.__fields__.keys()]
    ))

    return addToGoogleSheets(case_dict)
...
def clear_case():
    for key in ['case_description', 'case_title', 'result']:
        st.session_state[key] = ""
    update_case_ID()
...
def fetch_case_ids_and_titles():
    try:
        ...
        return [(row["Case ID"], row["Title"]) for row in data if "Case ID" in row and "Title" in row]
    except Exception as e:
...
def fetch_case_ids():
    return [case_id for case_id in get_google_sheet("2024 Healthtech Identify Log", "Case Log").col_values(1)[1:]]
...
if action == "Add New Case":
    ...
    def update_case_ID():
        ...
        case_sheet = get_google_sheet("2024 Healthtech Identify Log", "Case Log")
        case_date_ids = [case_id for case_id in case_sheet.col_values(1) if case_id.startswith(f"CA{case_date_str}")]
        counter = int(case_date_ids[-1][-4:]) + 1 if case_date_ids else 1
    ...
    with col1:
        if st.button("Submit Case"):
            ...
            if st.session_state['case_title']:
                with st.container():
                    st.text_area("Case Title (editable):", value=st.session_state['case_title'], height=100, key="case_title_edit")
            st.session_state['result'] = extractCaseFeatures(st.session_state['case_description'])
    ...
    if st.button("Log Case", disabled=not st.session_state['case_title']):
        if st.session_state['case_description']:
            ...
            if status:
                st.success("Case added to your team's database.")
                st.session_state['rerun'] = True
                st.rerun()
            else:
                st.error("Error adding case to the database.")
        else:
    ...
    if not case_info:
        ...
        selected_case = st.selectbox("Select a case to edit", [f"{case_id}: {title}" for case_id, title in case_info])
