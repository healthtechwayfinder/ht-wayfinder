...
#     title: Optional[str] = Field(default=None, description="One sentence summary of the observation as a unique title...")
...
#     people_present: Optional[str] = Field(default=None, description="names cited in the case description")
...
class CaseRecord(BaseModel):
    stakeholders: Optional[str] = Field(default=None, description="Stakeholders involved in the observation.")
    insider_language: Optional[str] = Field(default=None, description="Terminology specific to the procedure.")
    tags: Optional[str] = Field(default=None, description="Keywords or tags for the case.")
...
# Fetch case details based on case ID
def fetch_case_details(case_id):
    sheet = get_google_sheet("2024 Healthtech Identify Log", "Case Log")
    data = sheet.get_all_records()
    for row in data:
...
# LLM and Pinecone functions
def parse_case(case_description: str):
    llm = ChatOpenAI(model_name="gpt-4o", temperature=0.7, openai_api_key=OPENAI_API_KEY, max_tokens=500)
    case_prompt = PromptTemplate.from_template("""
        Parse descriptions of medical procedures to extract details such as surgeon, procedure, and date.
        case_description: {case_description}
        Output:
    """)
...
def generate_case_summary(case_description):
    llm = ChatOpenAI(model_name="gpt-4o", temperature=0.7, openai_api_key=OPENAI_API_KEY, max_tokens=500)
    case_prompt = PromptTemplate.from_template("""
        Create a brief 4-10 word title for the following medical case description.
        case_description: {case_description}
        Output title:
    """)
...
def embed_case(attendees, case_description, case_summary, case_date, case_id):
    db = PineconeVectorStore(index_name=st.secrets["pinecone-keys"]["index_to_connect"], namespace="cases",
                             embedding=OpenAIEmbeddings(api_key=OPENAI_API_KEY), pinecone_api_key=st.secrets["pinecone-keys"]["api_key"])
    db.add_texts([case_description], metadatas=[{'attendees': attendees, 'case_date': case_date, 'case_ID': case_id}])
    st.success("Case added to Pinecone")
        ...
        if case_to_edit:
            ...
            if case_details:
                ...
                try:
                    ...
                    st.error(f"Invalid date format: {case_date_str}")
                    case_date_edit = st.date_input("Edit Case Date", date.today())

                if st.button("Save Changes"):
                    updated_data = {"Case Title": case_title_edit, "Case Description": case_description_edit, "Case Date": case_date_edit.isoformat()}
                    if update_case(case_to_edit, updated_data):
                        st.success(f"Changes to '{case_to_edit}' saved successfully!")
                    else:
                        st.error(f"Failed to save changes to '{case_to_edit}'.")
